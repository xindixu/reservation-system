    ...
    >
      <Form.Item label="Client">
        <Select onChange={client => setVisit({ ...visit, client })}>
          {data ? data.clients.map(({ id, firstName, lastName }) => (
      ...
      <Form.Item>
        <Checkbox checked={visit.allDay} onChange={e => setVisit({ ...visit, allDay: e.target.checked })}>All day</Checkbox>
      </Form.Item>
      <Form.Item label="Note">
        <Input.TextArea rows={5} allowClear onChange={e => setVisit({ ...visit, note: e.target.value })} />
      </Form.Item>
    </Form>
  )
}
